...
class TaskEvaluationStrategy(ABC):
...
class AllTasksMustSucceedStrategy(TaskEvaluationStrategy):
...
class AnyTaskMustSucceedStrategy(TaskEvaluationStrategy):
...
class MajorityTasksMustSucceedStrategy(TaskEvaluationStrategy):
...
class MinimumSuccessThresholdStrategy(TaskEvaluationStrategy):
...
class PercentageSuccessThresholdStrategy(TaskEvaluationStrategy):
...
class NoFailuresAllowedStrategy(TaskEvaluationStrategy):
...
class EventEvaluator:
    ...
    def __init__(self, strategy: TaskEvaluationStrategy):
        """
        Args:
    ...
    def evaluate(self, task_results: List[TaskResult]) -> EventEvaluationResult:
        """
...
if __name__ == "__main__":
    # Create some sample task results
    task_results = [
        TaskResult(task_id="task_1", success=True, data="result1"),
        TaskResult(task_id="task_2", success=False, error=Exception("Task 2 failed")),
        TaskResult(task_id="task_3", success=True, data="result3"),
        TaskResult(task_id="task_4", success=False, error=Exception("Task 4 failed")),
        TaskResult(task_id="task_5", success=True, data="result5"),
    ]

    # Test different evaluation strategies
    print("=== Event Evaluation Results ===\n")

    # Strict evaluation (all must succeed)
    strict_evaluator = EventEvaluatorFactory.strict_evaluator()
    result = strict_evaluator.evaluate(task_results)
    print(f"Strategy: {result.strategy_used}")
    print(f"Success: {result.success}")
    print(f"Success Rate: {result.success_rate:.1f}%")
    print(f"Tasks: {result.successful_tasks}/{result.total_tasks} succeeded\n")

    # Lenient evaluation (any must succeed)
    lenient_evaluator = EventEvaluatorFactory.lenient_evaluator()
    result = lenient_evaluator.evaluate(task_results)
    print(f"Strategy: {result.strategy_used}")
    print(f"Success: {result.success}")
    print(f"Success Rate: {result.success_rate:.1f}%")
    print(f"Tasks: {result.successful_tasks}/{result.total_tasks} succeeded\n")

    # Percentage-based evaluation (60% must succeed)
    percentage_evaluator = EventEvaluatorFactory.percentage_evaluator(60.0)
    result = percentage_evaluator.evaluate(task_results)
    print(f"Strategy: {result.strategy_used}")
    print(f"Success: {result.success}")
    print(f"Success Rate: {result.success_rate:.1f}%")
    print(f"Tasks: {result.successful_tasks}/{result.total_tasks} succeeded\n")
    ...
    custom_evaluator = EventEvaluator(MinimumSuccessThresholdStrategy(2))
    result = custom_evaluator.evaluate(task_results)
    print(f"Strategy: {result.strategy_used}")
    print(f"Success: {result.success}")
    print(f"Success Rate: {result.success_rate:.1f}%")
    print(f"Tasks: {result.successful_tasks}/{result.total_tasks} succeeded")
