...
class AbstractTelemetryLogger(ABC):
    ...
    @abstractmethod
    def record_retry(self, task_id: str, name: str) -> None:
...
class BatchTelemetryLogger:
    ...
    def __init__(self):
        """
        The _metrics is a dictionary that maps task_id to a list of EventMetrics
        Each EventMetrics object represents a single event execution
        The task_id is a unique identifier for each event execution. It is generated by the runtime and
        during batch processing, events with the same task_id are events of the same type.
        """
        self._metrics: typing.Dict[Task_Id, typing.List[EventMetrics]] = {}
        self._lock = threading.Lock()
        self._publishers: typing.List[MetricsPublisher] = []
    ...
    def start_event(
        ...
        """Record the start of an event execution"""
        with self._lock:
            self._metrics.setdefault(task_id, []).append(
                EventMetrics(
                    event_name=event_name,
                    task_id=task_id,
                    start_time=time.time(),
    ...
    def end_event(
        self, task_id: str, name: str, error: typing.Optional[str] = None
    ) -> None:
        """Record the end of an event execution"""
        with self._lock:
            if task_id in self._metrics:
                pipline_metrics = self._metrics[task_id]
                metrics = next(p for p in pipline_metrics if p.event_name == name)
    ...
    def get_metrics(self, task_id: str) -> typing.Optional[typing.List[EventMetrics]]:
        """Get metrics for a specific task"""
        with self._lock:
            return self._metrics.get(task_id, [])

    def get_all_metrics(self) -> typing.Dict[str, typing.List[EventMetrics]]:
        """Get all collected metrics"""
        with self._lock:
            return self._metrics.copy()
